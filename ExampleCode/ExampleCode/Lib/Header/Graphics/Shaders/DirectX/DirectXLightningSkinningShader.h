#ifndef DIRECTXLIGHTNINGSKINNINGSHADER_H
#define DIRECTXLIGHTNINGSKINNINGSHADER_H
...
namespace BAME
{
	class DirectXLightningSkinningShader : public DirectXShader
	{
	public:
		DirectXLightningSkinningShader(Microsoft::WRL::ComPtr<ID3D11Device> const& device, Microsoft::WRL::ComPtr<ID3D11DeviceContext> const& deviceContext);
		~DirectXLightningSkinningShader();
		...
		void UpdateBoneList(std::vector<Matrix4x4f>const& boneMatrixList);
	...
	private:
		...
		//std::vector<Matrix4x4f> mBoneMatrixList;
		...
		struct alignas(16) cbPerFrame
		{
			...
			XMMATRIX mGlobalInverseTranspose;
		};
		...
		struct alignas(16) cbBoneProperty
		{
			XMMATRIX mBones[60];
		};
